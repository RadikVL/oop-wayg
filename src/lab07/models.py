"""
Иерархия счетов — копия из ЛР-6 (с типами и методами display/score под
протоколы). Тащим прямо сюда, потому что лаба должна запускаться сама,
без хитрых импортов из других папок. Предметку не меняем — таково условие.
"""
